...
CIF_res1 <- function(initial_params = rep(0.001, 4), Method = "BFGS") {
  ...
  # Enforce numerical symmetry for stability
  Information_matrix <- (Information_matrix + t(Information_matrix)) / 2

  # Check for corrupted matrix (e.g. NA, Inf)
  ...
  # Stable inversion with fallback and rescue mechanism
  invInfo <- tryCatch(
    solve(Information_matrix),
    error = function(e) {
      M <- Information_matrix
      lam <- 0; res <- NULL
      for (i in 1:8) {
        Mreg <- M + diag(lam, 4)
        res <- tryCatch(solve(Mreg), error = function(e) NULL)
        if (!is.null(res)) break
        lam <- if (lam == 0) 1e-8 else lam * 10
      }
      if (is.null(res)) {
        tryCatch(qr.solve(M, diag(4)), error = function(e) diag(NA_real_, 4))
      } else {
        (res + t(res)) / 2
      }
    }
  ...
  # Extract variances and clip negatives
  vars <- pmax(diag(invInfo), 0)
...
# Simulation I | Scenario I: $\gamma (\alpha):~ -0.1, \quad \lambda (\beta): ~ 0.012, \quad \text{censor\_rate}: 0.3
...
# Simulation I | Scenario II: $\gamma (\alpha):~ 0.1, \quad \lambda (\beta): ~ 0.012, \quad \text{censor\_rate}: 0.3
...
# Simulation I | Scenario III: $\gamma (\alpha):~ -0.1, \quad \lambda (\beta): ~ 0.012, \quad \text{censor\_rate}: 0.5
...
# Simulation I | Scenario IV: $\gamma (\alpha):~ 0.1, \quad \lambda (\beta): ~ 0.012, \quad \text{censor\_rate}: 0.5
...
# Simulation I | : $\gamma (\alpha):~ -0.1, \quad \lambda (\beta): ~ 0.012, \quad \text{censor\_rate}: 0.7
...
# Simulation I | Scenario VI: $\gamma (\alpha):~ 0.1, \quad \lambda (\beta): ~ 0.012, \quad \text{censor\_rate}: 0.7
...
# Simulation II | Scenario I: $\gamma (\alpha):~ -0.1, \quad \lambda (\beta): ~ 0.012, \quad \text{censor\_rate}: 0.3
...
# Simulation II | Scenario II: $\gamma (\alpha):~ 0.1, \quad \lambda (\beta): ~ 0.012, \quad \text{censor\_rate}: 0.3
...
# Simulation II | Scenario III: $\gamma (\alpha):~ -0.1, \quad \lambda (\beta): ~ 0.012, \quad \text{censor\_rate}: 0.5
...
# Simulation II | Scenario IV: $\gamma (\alpha):~ 0.1, \quad \lambda (\beta): ~ 0.012, \quad \text{censor\_rate}: 0.5
...
# Simulation II | : $\gamma (\alpha):~ -0.1, \quad \lambda (\beta): ~ 0.012, \quad \text{censor\_rate}: 0.7
...
# Simulation II | Scenario VI: $\gamma (\alpha):~ 0.1, \quad \lambda (\beta): ~ 0.012, \quad \text{censor\_rate}: 0.7
